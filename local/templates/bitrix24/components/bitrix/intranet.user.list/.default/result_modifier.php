...
$showYearValue = Option::get("intranet", "user_profile_show_year", "N");
	...
	foreach($gridColumns as $column)
	{
		switch($column)
		{
            ...
            case 'UF_USR_1683828500926':
                $sp = $USER->getById($arResult['ROWS'][$key]['id'])->GetNext();
                if ($sp['UF_USR_1683828500926'] > 0) {
                    $sp_link = '<a href="callto://'.$sp['UF_USR_1683828500926'].'" onclick="if(typeof(top.BXIM) !== \'undefined\') { top.BXIM.phoneTo(\''.$sp['UF_USR_1683828500926'].'\', {}); return BX.PreventDefault(event); }">
                                <div class="ui-entity-editor-content-block-text" style="display: inline-block;">'.$sp['UF_USR_1683828500926'].'</div><div class="short-phone-icon"></div>
                            </a>';
                } else {
                    $sp_link = '';
                }
                $arResult['ROWS'][$key]['data'][$column] = $sp_link;
                break;
            case 'UF_PHONE_INNER':
                $spi = $USER->getById($arResult['ROWS'][$key]['id'])->GetNext();
                if ($spi['UF_PHONE_INNER'] > 0) {
                    $sp_link = '<a href="callto://'.$spi['UF_PHONE_INNER'].'" onclick="if(typeof(top.BXIM) !== \'undefined\') { top.BXIM.phoneTo(\''.$spi['UF_PHONE_INNER'].'\', {}); return BX.PreventDefault(event); }">
                                <div class="ui-entity-editor-content-block-text" style="display: inline-block;">'.$spi['UF_PHONE_INNER'].'</div><div class="short-phone-icon"></div>
                            </a>';
                } else {
                    $sp_link = '';
                }
