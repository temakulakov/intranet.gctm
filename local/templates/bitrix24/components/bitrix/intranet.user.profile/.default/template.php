...
<div class="intranet-user-profile" id="intranet-user-profile-wrap">
	...
	<div class="intranet-user-profile-column-right">
		...
		//endregion
        array_push($arResult['UserFieldsAvailable'], 'UF_USR_1683828500926');
        $arUfPhoneFormFields = [
            'title' => 'Групповой внутренний номер',
            'name' => 'UF_USR_1683828500926',
            'type' => 'text',
            'editable' => 1,
            'showAlways' => 1
        ];
        array_push($arResult['FormFields'], $arUfPhoneFormFields);
        array_push($arResult['FormConfig'][0]['elements'], ['name' => 'UF_USR_1683828500926']);
        ...
        $rsUser = $USER->GetByID($arResult['User']['ID']);
        $arUser = $rsUser->Fetch();
        $grtel = $arUser['UF_USR_1683828500926'];
        $vntel = $arUser['UF_PHONE_INNER'];
		if (
			$showRuNotification
...
<script>
    function ready() {
        document.querySelector('[data-cid="UF_USR_1683828500926"] .ui-entity-editor-content-block-text').innerHTML = '<a href="callto://<?=$grtel?>" onclick="if(typeof(top.BXIM) !== \'undefined\') { top.BXIM.phoneTo(\'<?=$grtel?>\', {}); }"> <div class="ui-entity-editor-content-block-text" style="display: inline-block;"><?=$grtel?></div> <div class="intranet-user-profile-phone-icon"></div></a>';
        document.getElementsByClassName('side-panel-iframe')[0].contentDocument.querySelector('[data-cid="UF_PHONE_INNER"] .ui-entity-editor-content-block').innerHTML = '<a href="callto://<?=$vntel?>" onclick="if(typeof(top.BXIM) !== \'undefined\') { top.BXIM.phoneTo(\'<?=$vntel?>\', {}); }"> <div class="ui-entity-editor-content-block-text" style="display: inline-block;"><?=$vntel?></div> <div class="intranet-user-profile-phone-icon"></div></a>';
    }
