...
//Переименование
foreach ($arResult["ITEMS"] as $shk => $sh) {
    foreach ($sh as $mik => $mItem) {
        if ($mItem['TEXT'] == 'Компания') {
            $arResult["ITEMS"][$shk][$mik]['TEXT'] = 'Сотрудники';
        }
        if ($mItem['TEXT'] == 'Группы') {
            $arResult["ITEMS"][$shk][$mik]['TEXT'] = 'Проекты';
        }
        if ($mItem['TEXT'] == 'Почта') {
            $arResult["ITEMS"][$shk][$mik]['TEXT'] = 'Почта (бета)';
        }
        if ($mItem['TEXT'] == 'Почта Mail.ru') {
            $arResult["ITEMS"][$shk][$mik]['TEXT'] = 'Почта';
        }
...
if ($isAdmin != 1) {
    foreach($arResult["ITEMS"]['show'] as $ik => $item) {
       // menu_sites menu_contact_center
        if ($item['ID'] == 'menu_sites' || $item['ID'] == 'menu_contact_center') {
            unset($arResult["ITEMS"]['show'][$ik]);
        }
    }
}