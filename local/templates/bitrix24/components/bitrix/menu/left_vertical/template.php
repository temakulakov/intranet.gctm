...
<div class="menu-items-block menu-items-view-mode" id="menu-items-block">
    <div class="menu-items-header">
        <?
        include($_SERVER["DOCUMENT_ROOT"] . "/local/templates/bitrix24/logo.php");
        ?>
    ...
    <div class="menu-items-body">
        <div class="menu-items-body-inner">
            ...
            foreach ($arResult["ITEMS"][$status] as $item) {
                ...
                if (strpos($curLink, 'e1c://server/zeon/edo3') !== false)
                    $curLink = 'e1c://server/zeon/edo3';
            ...
            ?>
                <li id="bx_left_menu_<?= $itemId ?>" data-status="<?= $status ?>" data-id="<?= $item["PARAMS"]["menu_item_id"] ?>" data-role="<?= isset($item['IS_GROUP']) && $item['IS_GROUP'] === 'Y' ? 'group' : 'item' ?>" <? if (isset($item['IS_GROUP']) && $item['IS_GROUP'] === 'Y') : ?> data-collapse-mode="<?= $item['PARAMS']['collapse_mode'] ?>" <? endif ?> data-storage="<?= $item['PARAMS']['storage'] ?? '' ?>" data-counter-id="<?= $counterId ?>" data-link="<?= $curLink ?>" data-all-links="<?= $addLinks ?>" data-type="<?= $item["ITEM_TYPE"] ?>" data-delete-perm="<?= $item["DELETE_PERM"] ?>" <? if (isset($item["PARAMS"]["is_application"])) : ?> data-app-id="<?= $item["PARAMS"]["app_id"] ?>" <? endif ?> <? if (isset($item["PARAMS"]["top_menu_id"])) : ?> data-top-menu-id="<?= $item["PARAMS"]["top_menu_id"] ?>" <? endif ?> data-new-page="<?= (isset($item["OPEN_IN_NEW_PAGE"]) && $item["OPEN_IN_NEW_PAGE"] === "Y" ? "Y" : "N") ?>" <? if (array_key_exists("can_be_first_item", $item["PARAMS"]) && !$item["PARAMS"]["can_be_first_item"]) : ?> data-disable-first-item="Y" <? endif ?> class="<?= $itemClass ?>">
                    ...
                    ?>
                    <a class="menu-item-link" href="<?= (isset($item["PARAMS"]["onclick"])) ? "javascript:void(0)" : $curLink ?>" <? if (isset($item["OPEN_IN_NEW_PAGE"]) && ($item["OPEN_IN_NEW_PAGE"] === "Y")) : ?> target="_blank" <? endif ?> <?= (mb_strpos($curLink, SITE_DIR . 'workgroups/group/') === 0 ? 'data-slider-ignore-autobinding="true"' : '') ?> onclick="<? if (isset($item["PARAMS"]["onclick"])) : ?>
                    ...
                    <? endif ?>">
                        ...
                        if ($counterId <> '') {
                            ...
                            <span class="menu-item-index-wrap">
										<span data-role="counter" data-counter-value="<?= $valueCounter ?>" class="menu-item-index" id="menu-counter-<?= mb_strtolower($item["PARAMS"]["counter_id"]) ?>"><?= $badgeCounter ?></span>
									</span>
                            <?
                        }
                        if (isset($item['IS_GROUP']) && $item['IS_GROUP'] === 'Y') : ?>
            ...
            <div class="menu-favorites-more-btn<? if (empty($arResult["ITEMS"]["hide"])) : ?> menu-favorites-more-btn-hidden<? endif ?>">
                ...
                <div class="menu-default-more-btn">
                    <span class="menu-favorites-more-text" id="menu-more-btn-text"><?= Loc::getMessage("MENU_MORE_ITEMS_SHOW") ?></span>
                    <span class="menu-favorites-more-icon"></span>
                </div>
            ...
            <div class="menu-extra-btn-box">
                ...
                if ($arResult["SHOW_LICENSE_BUTTON"]) :
                    ...
                    <div class="menu-license-all-container">
						<span class="menu-license-all menu-license-all-collapsed" onclick="
                        ...
                        <? if ($arResult["IS_DEMO_LICENSE"] && !empty($arResult["DEMO_DAYS"])) : ?>
                            <span class="menu-license-all menu-license-all-default" onclick="
                                    if (BX.getClass('B24.upgradeButtonRedirect'))
                                    B24.upgradeButtonRedirect(<?= CUtil::PhpToJSObject($arJsParams) ?>)">
								<span class="menu-license-all-icon"></span>
    ...
    <div class="group-panel-content group-panel-content-<?= $filter ?>" data-filter="<?= $filter ?>" id="group-panel-content">
        <div class="group-panel-header">
			<span class="group-panel-header-filters">
				<span class="group-panel-header-filter group-panel-header-filter-all" data-filter="all">Мои проекты<?/*= GetMessage("MENU_MY_WORKGROUPS")*/ ?></span>
				<? if (isModuleInstalled("extranet")) : ?>
                    <span class="group-panel-header-filter group-panel-header-filter-extranet" data-filter="extranet"><?= GetMessage("MENU_MY_WORKGROUPS_EXTRANET") ?></span>
                <? endif ?>
				<span class="group-panel-header-filter group-panel-header-filter-favorites" data-filter="favorites"><?= GetMessage("MENU_MY_WORKGROUPS_FAVORITES") ?>
        ...
        <div class="group-panel-items" id="group-panel-items">
            ...
            foreach ($arResult["GROUPS"] as $index => $group) :
                ...
                <a href="<?= SITE_DIR ?>workgroups/group/<?= $group["ID"] ?>/" class="<?= $className ?>" data-id="<?= $group["ID"] ?>" data-slider-ignore-autobinding="true">
                    <span class="group-panel-item-text" title="<?= htmlspecialcharsbx($group["NAME"]) ?>"><?= htmlspecialcharsbx($group["NAME"]) ?></span>
...
<script>
    let cal = $("bx_left_menu_2940362083");
    let journ =  $("bx_left_menu_3050064345");
    switch(window.location.href) {
        // Для календарей
        case 'https://intranet.gctm.ru/calendar/':
            $("#bx_left_menu_2940362083").addClass("menu-item-active");

            break;
        case 'https://intranet.gctm.ru/calendar/rooms/':
            console.log(window.location.href);
            $("#bx_left_menu_2940362083").addClass("menu-item-active");
            break;

        // Для Журнала учета выездов
        case 'https://intranet.gctm.ru/komandirovki/':
            $("#bx_left_menu_3050064345").addClass("menu-item-active");
            break;
        case 'https://intranet.gctm.ru/services/lists/?mode=edit&list_id=64&section_id=0&element_id=0&list_section_id=':
            $("#bx_left_menu_3050064345").addClass("menu-item-active");
            break;
        case 'https://intranet.gctm.ru/company/personal/bizproc/':
            $("#bx_left_menu_3050064345").addClass("menu-item-active");
            break;
        case 'https://intranet.gctm.ru/komandirovki/history.php':
            $("#bx_left_menu_3050064345").addClass("menu-item-active");
            break;
        case 'https://intranet.gctm.ru/services/lists/?mode=view&list_id=64&section_id=0&list_section_id=':
            $("#bx_left_menu_3050064345").addClass("menu-item-active");
            break;
    }
</script>
