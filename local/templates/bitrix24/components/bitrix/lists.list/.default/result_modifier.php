...
if (str_contains( $_SERVER['REQUEST_URI'], "/komandirovki/history.php")) {
    foreach ($cp->arResult['ELEMENTS_ROWS'] as $key => $value) {
        preg_match_all("/\d+/", $cp->arResult['ELEMENTS_ROWS'][$key]['columns']['PROPERTY_280'], $matches);
        if ($matches[0][0] != $USER->GetID()) {
            unset($cp->arResult['ELEMENTS_ROWS'][$key]);
        }
    }
} elseif (str_contains( $_SERVER['REQUEST_URI'], "/komandirovki/")) {
    foreach ($cp->arResult['ELEMENTS_ROWS'] as $key => $value) {
